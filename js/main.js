...
async function buyCard(c, free) {
    ...
    if (!isTuto || tutoCard && tutoCard.card.effects.findIndex(e => [-4, -13, -21].includes(e.id)) != -1) {
        ...
        if (c.card.species !== "Sortilège")
            speciesBought[c.card.species] = true;
        if (round <= 2)
            lazy = false;
...
async function dragDrop() {
    ...
    if (document.getElementById("hand").contains(card) && this.className === "spell-area" && card.card.species == "Sortilège" && card.card.validTarget.area === "any")
        ...
        card.parentElement.removeChild(card);
    } else if (document.getElementById("hand").contains(card) && this.name === "position" && this.children[0] && players[0].effects.some(e => e.id === 69) && card.card.species !== "Sortilège" && card.card.species !== this.children[0].card.species) {
        this.children[0].card.species = card.card.species;
        boostStats(this.children[0].card, 1, 1, true);
        card.parentElement.removeChild(card);
    }
}
...
async function drawShopScene() {
    ...
    if (players[0].hp <= 0) {
        ...
        if (!JSON.parse(window.localStorage.getItem("Achievement__Hyperspécialisation")) && Object.keys(speciesBought).length <= 1)
            newAchievements.push("Hyperspécialisation");
        if (!JSON.parse(window.localStorage.getItem("Achievement__Honneur du guerrier")) && noSpell)
            newAchievements.push("Honneur du guerrier");
        if (!JSON.parse(window.localStorage.getItem("Achievement__Paresseux")) && lazy)
            newAchievements.push("Paresseux");
        ...
        round++;
        const roundForLevelUp = round + players[0].effects.some(e => e.id === 68);
        switch(roundForLevelUp) {
            case 3:
            case 5:
            case 8:
        ...
        document.getElementById("refresh").style.removeProperty("pointer-events");
        spendCoins(-Math.min(10, round + 2 - players[0].effects.some(e => e.id === 68)), true);
        if (species.includes("Loup-Garou")) {
...
function initCards() {
    ...
    for (let c of cardList["Commandant"]) {
        let card = createCard(c);
        if ((!card.requirement && Math.random() < .5) || species.includes(card.requirement)) //!!!
...
function createCard(card) {
    switch (card) {
        ...
        case "protecmage-radiant":
            return new ProtecmageRadiant();
        case "commandante-charismatique":
            return new CommandanteCharismatique();
        case "marchand-prospere":
            return new MarchandProspere();
        case "heros-de-guerre":
            return new HerosDeGuerre();
        case "vendeur-d-artefacts":
            return new VendeurDArtefacts();
        case "zoologiste-curieux":
            return new ZoologisteCurieux();
        case "exploratrice-celeste":
            return new ExploratriceCeleste();
        case "mentor-bienveillant":
            return new MentorBienveillant();
        case "noble-decadent":
        ...
        case "gardien-obstine":
            return new GardienObstine();
        case "agent-sans-visage":
            return new AgentSansVisage();
        case "maitre-horloger":
            return new MaitreHorloger();
        case "geneticien-elfe":
            return new GeneticienElfe();
        case "adepte-du-gel-eternel":
        ...
        case "vampire-assoiffee":
            return new VampireAssoiffee();
        case "chercheuse-de-sang":
            return new ChercheuseDeSang();
        case "vampire-gloutonne":
            return new VampireGloutonne();
        case "assassin-repu":
            return new AssassinRepu();
        case "guide-impie":
            return new GuideImpie();
        case "collecteur-de-sang":
            return new CollecteurDeSang();
        case "peintresse-ecarlate":
            return new PeintresseEcarlate();
        case "invite-inattendu":
            return new InviteInattendu();
        case "ecorcheuse-au-clair-de-lune":
            return new EcorcheuseAuClairDeLune();
        case "hematomancienne":
            return new Hematomancienne();
        case "chasseuse-vampire":
            return new ChasseuseVampire();
        case "approvisionneur-de-sang":
            return new ApprovisionneurDeSang();
        case "baron-sanglant":
            return new BaronSanglant();
        case "ame-assoiffee":
            return new AmeAssoiffee();
        case "cavalier-de-la-nuit":
            return new CavalierDeLaNuit();
        case "sang-impie":
            return new SangImpie();
...
function ProtecmageRadiant() {
    this.name = "Protecmage radiant";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 25;
    this.src = "commandants/protecmage-radiant.jpg";
    this.effects = [
        {
            trigger: "turn-start",
            id: 52
...
function CommandanteCharismatique() {
    this.name = "Commandante charismatique";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 29;
    this.src = "commandants/commandante-charismatique.jpg";
    this.effects = [
        {
            trigger: "battle-start",
            id: 53
...
function MarchandProspere() {
    this.name = "Marchand prospère";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 31;
    this.src = "commandants/marchand-prospere.jpg";
    this.effects = [
        {
            trigger: "turn-end",
            id: 54
...
function HerosDeGuerre() {
    this.name = "Héros de guerre";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 34;
    this.src = "commandants/heros-de-guerre.jpg";
    this.effects = [
        {
            trigger: "ko",
            id: 55
...
function VendeurDArtefacts() {
    this.name = "Vendeur d'artefacts";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 33;
    this.src = "commandants/vendeur-d-artefacts.jpg";
    this.effects = [
        {
            trigger: "turn-start",
            id: 57
...
function ZoologisteCurieux() {
    this.name = "Zoologiste curieux";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 34;
    this.src = "commandants/zoologiste-curieux.jpg";
    this.effects = [
        {
            trigger: "card-sell",
            id: 58
...
function ExploratriceCeleste() {
    this.name = "Exploratrice céleste";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 30;
    this.src = "commandants/exploratrice-celeste.jpg";
    this.effects = [
        {
            trigger: "shop-refresh",
            id: 59
...
function MentorBienveillant() {
    this.name = "Mentor bienveillant";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 28;
    this.src = "commandants/mentor-bienveillant.jpg";
    this.effects = [
        {
            trigger: "turn-start",
            id: 60
...
function NobleDecadent() {
    this.name = "Noble décadent";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 40;
    this.src = "commandants/noble-decadent.jpg";
    this.requirement = "Vampire";
    this.effects = [
        {
            trigger: "turn-start",
            id: 61
...
function ValseuseDeMinuit() {
    this.name = "Valseuse de minuit";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 31;
    this.src = "commandants/valseuse-de-minuit.jpg";
    this.requirement = "Vampire";
    this.effects = [
        {
            trigger: "shop-refresh",
...
function GardienObstine() {
    this.name = "Gardien obstiné";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 30;
    this.src = "commandants/gardien-obstine.jpg";
    this.effects = [
        {
            trigger: "ko",
            id: 65
...
function AgentSansVisage() {
    this.name = "Agent sans visage";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 31;
    this.src = "commandants/agent-sans-visage.jpg";
    this.effects = [
        {
            trigger: "card-place",
            id: 67
...
function MaitreHorloger() {
    this.name = "Maître horloger";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 28;
    this.src = "commandants/maitre-horloger.jpg";
    this.effects = [
        {
            trigger: "",
            id: 68
...
function GeneticienElfe() {
    this.name = "Généticien elfe";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 32;
    this.src = "commandants/geneticien-elfe.jpg";
    this.effects = [
        {
            trigger: "",
            id: 69
...
function AdepteDuGelEternel() {
    this.name = "Adepte du gel éternel";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 31;
    this.src = "commandants/adepte-du-gel-eternel.jpg";
    this.effects = [
        {
            trigger: "turn-end",
            id: 70
...
function AvatarDeLaFurie() {
    this.name = "Avatar de la furie";
    this.species = "Commandant";
    this.attack = -1;
    this.hp = 30;
    this.src = "commandants/avatar-de-la-furie.jpg";
    this.effects = [
        {
            trigger: "turn-start",
            id: 71
...
function VampireAssoiffee() {
    this.name = "Vampire assoiffée";
    this.species = "Vampire";
    this.attack = 4;
    this.hp = 3;
    this.src = "vampires/vampire-assoiffee.jpg";
    this.tier = 1;
    this.effects = [
        {
            trigger: "card-place",
...
function ChercheuseDeSang() {
    this.name = "Chercheuse de sang";
    this.species = "Vampire";
    this.attack = 3;
    this.hp = 2;
    this.src = "vampires/chercheuse-de-sang.jpg";
    this.tier = 1;
    this.effects = [
        {
            trigger: "card-place",
...
function VampireGloutonne() {
    this.name = "Vampire gloutonne";
    this.species = "Vampire";
    this.attack = 3;
    this.hp = 3;
    this.src = "vampires/vampire-gloutonne.jpg";
    this.tier = 2;
    this.effects = [
        {
            trigger: "turn-start",
...
function AssassinRepu() {
    this.name = "Assassin repu";
    this.species = "Vampire";
    this.attack = 4;
    this.hp = 3;
    this.src = "vampires/assassin-repu.jpg";
    this.tier = 2;
    this.effects = [
        {
            trigger: "tookdamage",
...
function GuideImpie() {
    this.name = "Guide impie";
    this.species = "Vampire";
    this.attack = 2;
    this.hp = 5;
    this.src = "vampires/guide-impie.jpg";
    this.tier = 2;
    this.effects = [
        {
            trigger: "turn-start",
...
function MorsureDeVampire() {
    this.name = "Morsure de vampire";
    this.species = "Sortilège";
    this.attack = -1;
    this.hp = -1;
    this.src = "vampires/morsure-de-vampire.jpg";
    this.tier = 2;
    this.requirement = "Vampire";
    this.effects = [
        {
...
function CollecteurDeSang() {
    this.name = "Collecteur de sang";
    this.species = "Vampire";
    this.attack = 2;
    this.hp = 4;
    this.src = "vampires/collecteur-de-sang.jpg";
    this.tier = 3;
    this.effects = [
        {
            trigger: "lifeloss",
...
function PeintresseEcarlate() {
    this.name = "Peintresse écarlate";
    this.species = "Vampire";
    this.attack = 3;
    this.hp = 4;
    this.src = "vampires/peintresse-ecarlate.jpg";
    this.tier = 3;
    this.effects = [
        {
            trigger: "card-place",
...
function InviteInattendu() {
    this.name = "Invité inattendu";
    this.species = "Vampire";
    this.attack = 4;
    this.hp = 4;
    this.src = "vampires/invite-inattendu.jpg";
    this.tier = 3;
    this.effects = [
        {
            trigger: "card-place",
...
function SangImpie() {
    this.name = "Sang impie";
    this.species = "Sortilège";
    this.attack = -1;
    this.hp = -1;
    this.src = "vampires/sang-impie.jpg";
    this.tier = 3;
    this.requirement = "Vampire";
    this.effects = [
        {
...
function EcorcheuseAuClairDeLune() {
    this.name = "Ecorcheuse au clair de lune";
    this.species = "Vampire";
    this.attack = 4;
    this.hp = 5;
    this.src = "vampires/ecorcheuse-au-clair-de-lune.jpg";
    this.tier = 4;
    this.effects = [
        {
            trigger: "lifeloss",
...
function Hematomancienne() {
    this.name = "Hématomancienne";
    this.species = "Vampire";
    this.attack = 1;
    this.hp = 3;
    this.src = "vampires/hematomancienne.jpg";
    this.tier = 4;
    this.effects = [
        {
            trigger: "battle-start",
...
function ChasseuseVampire() {
    this.name = "Chasseuse vampire";
    this.species = "Vampire";
    this.attack = 6;
    this.hp = 3;
    this.src = "vampires/chasseuse-vampire.jpg";
    this.tier = 4;
    this.range = true;
    this.effects = [
        {
...
function ApprovisionneurDeSang() {
    this.name = "Approvisionneur de sang";
    this.species = "Vampire";
    this.attack = 6;
    this.hp = 6;
    this.src = "vampires/approvisionneur-de-sang.jpg";
    this.tier = 5;
    this.effects = [
        {
            trigger: "attack",
...
function CavalierDeLaNuit() {
    this.name = "Cavalier de la nuit";
    this.species = "Vampire";
    this.attack = 4;
    this.hp = 6;
    this.src = "vampires/cavalier-de-la-nuit.jpg";
    this.tier = 5;
    this.effects = [
        {
            trigger: "card-place",
...
function BaronSanglant() {
    this.name = "Baron sanglant";
    this.species = "Vampire";
    this.attack = 8;
    this.hp = 8;
    this.src = "vampires/baron-sanglant.jpg";
    this.tier = 6;
    this.effects = [
        {
            trigger: "card-sell",
...
function AmeAssoiffee() {
    this.name = "Âme assoiffée";
    this.species = "Vampire";
    this.attack = 7;
    this.hp = 2;
    this.src = "vampires/ame-assoiffee.jpg";
    this.tier = 6;
    this.revive = true;
    this.effects = [
        {
...
function BouclierRadiant() {
    this.name = "Bouclier radiant";
    this.species = "Sortilège";
    this.attack = -1;
    this.hp = -1;
    this.src = "sortileges/bouclier-radiant.jpg";
    this.tier = 7;
    this.effects = [
        {
            trigger: "",
...
function createEffect(id) {
    switch (id) {
        ...
        case 1701:
            return new Effect1701();
        case 1702:
            return new Effect1702();
        case 1703:
            return new Effect1703();
        case 1704:
            return new Effect1704();
        case 1705:
            return new Effect1705();
        case 1706:
            return new Effect1706();
        case 1707:
            return new Effect1707();
        case 1708:
            return new Effect1708();
        case 1709:
            return new Effect1709();
        case 1710:
            return new Effect1710();
        case 1711:
            return new Effect1711();
        case 1712:
            return new Effect1712();
        case 1713:
            return new Effect1713();
        case 1714:
            return new Effect1714();
        case 1715:
            return new Effect1715();
        case 1716:
            return new Effect1716();
        case 1717:
            return new Effect1717();
        case 1718:
            return new Effect1718();
        case 1719:
            return new Effect1719();
        case 1720:
...
function Effect52() {
    this.run = async (sender, args, doAnimate) => {
        if (!sender.effect52)
            sender.effect52 = 1;
        if (shopTier > sender.effect52) {
            sender.effect52 = shopTier;
            if (doAnimate)
                await effectProcGlow(sender);
            let card = new BouclierRadiant();
            card.created = true;
            let c = drawCard(card, 176);
            await addToHand(c);
        }
    ...
    this.desc = "Lorsque le niveau de recrutement augmente, ajoute à votre main un Bouclier radiant.";
    this.refs = ["bouclier-radiant"];
}
...
function Effect53() {
        ...
        if (sender.hp > 0) {
            ...
            for (let c of t[0].concat(t[1])) {
                if (c) {
                    ...
                    if (doAnimate)
                        await boostStats(c, 0, 0, doAnimate);
                    let side = args[0][0].concat(args[0][1]).includes(c);
                    let t = (side ? args[1][0].concat(args[1][1]) : args[0][0].concat(args[0][1]));
                    if (t.filter(e => e).length > 0)
                        await attack(args[0], args[1], args[2], args[3], args[4], doAnimate, c, choice(t.filter(e => e)));
                    break;
                }
            }
        }
    ...
    this.desc = "<em>Frappe préventive :</em> La première créature alliée acquiert <em>Bouclier</em>, puis attaque une créature ennemie aléatoire.";
}
...
function Effect54() {
    this.run = async (sender, args, doAnimate) => {
        if (coins) {
            if (doAnimate)
                await effectProcGlow(sender);
            const t = shuffle([...troops[0]]).filter(e => e);
    ...
    this.desc = "A la fin de chaque tour, confère +1/+1 à une créature alliée aléatoire différente pour chaque pièce en votre possession.";
}
...
function Effect55() {
    this.run = async (sender, args, doAnimate) => {
        ...
        if (!sender.effect55 && players[args[4]] === sender && !t.some(c => c && c.hp > 0)) {
            ...
            if (doAnimate)
                await effectProcGlow(sender);
            await battleSummon("heros-de-guerre-t", args[1] ? args[2] : args[3], args[4], doAnimate, args);
        }
    };
    this.scaling = (c, t) => {
        return [0, 0, 0, 0];
...
function Effect57() {
    this.run = async (sender, args, doAnimate) => {
        if (!sender.effect57)
            sender.effect57 = 1;
        if (shopTier > sender.effect57) {
            sender.effect57 = shopTier;
            if (doAnimate)
                await effectProcGlow(sender);
            for (let i = 0; i < 2; i++) {
    ...
    this.desc = "Lorsque le niveau de recrutement augmente, ajoute 2 Pièces d'or à votre main.";
    this.refs = ["piece-d-or"];
}

function Effect58() {
    this.run = async (sender, args, doAnimate) => {
        const targets = troops[0].filter(c => c && c.name === args[0].card.name);
        if (targets.length) {
            if (doAnimate)
                await effectProcGlow(sender);
            for (let card of targets)
                boostStats(card, 2, 2, doAnimate);
            await sleep(400);
        }
    };
    this.scaling = (c, t) => {
    ...
    this.desc = "Lorsque vous revendez une créature, confère +2/+2 aux créatures alliées du même nom.";
}
...
function Effect59() {
    ...
    this.dynamicDesc = (c) => !c.effect59 || c.effect59 < 10 ? `<em>(Encore ${10 - (c.effect59 ?? 0)})</em>` : "<em>(Prochaine recherche)</em>";
    this.desc = "Après avoir recherché des recrues 10 fois, la prochaine recherche donne des recrues de plus haut rang.";
}

function Effect60() {
    this.run = async (sender, args, doAnimate) => {
        const t = troops[0].filter(c => c && c.effects.some(e => createEffect(e.id).desc.startsWith("<em>Recrue")));
        if (t.length) {
            if (doAnimate)
                await effectProcGlow(sender);
            const target = choice(t);
            for (let e of target.effects) {
                    let eff = createEffect(e.id);
                    if (eff.desc.startsWith("<em>Recrue"))
                        await eff.run(target, [findDOMCard(target)], true);
                }
        }
    };
    this.scaling = (c, t) => {
    ...
    this.desc = "Au début de chaque tour, redéclenche les <em>Recrues</em> d'une créature alliée aléatoire.";
}
...
function Effect61() {
    ...
    this.desc = "Au début de chaque tour, perd 1 PV.";
}
...
function Effect62() {
    this.run = async (sender, args, doAnimate) => {
        let shop = document.getElementById("shop");
        for (let c of shop.children) {
            if (c.classList.contains("card") && c.card.species !== "Sortilège" && c.card.species !== "Vampire" && Math.random() < .1) {
                c.card.species = "Vampire";
                c.card.effects.push({
                    trigger: "lifeloss",
                    id: 1721
                });
...
function Effect65() {
    this.run = async (sender, args, doAnimate) => {
        console.log(args[0].original, findDOMCard(args[0].original))
        if (players[args[4]] === sender && args[0].original && !args[0].original.effects.some(e => e.id === 323)) {
            args[0].original.attack++;
        }
    };
    this.scaling = (c, t) => {
        return [0, 0, 0, 0];
    ...
    this.desc = "Lorsqu'une créature alliée meurt, lui confère définitivement +1/+0.";
}
...
function Effect66() {
    this.run = async (sender, args, doAnimate) => {
        if (args[0] == 1 && players[args[3]] === sender || args[0] == 2 && players[args[4]] === sender) {
            if (doAnimate)
                await effectProcGlow(sender);
            let t = args[0] == 1 ? args[1][0].concat(args[1][1]) : args[2][0].concat(args[2][1]);
            for (let c of t.filter(e => e && e.hp > 0)) {
    ...
    this.desc = "Lorsque vous gagnez un combat, confère définitivement +0/+1 aux créatures alliées survivantes.";
}

function Effect67() {
    this.run = async (sender, args, doAnimate) => {
        if (args[0].card.species === "Autre") {
            if (doAnimate)
                await effectProcGlow(sender);
            args[0].card.species = choice(species);
            await boostStats(args[0].card, 0, 0, doAnimate);
        }
    };
    this.scaling = (c, t) => {
    ...
    this.desc = "Lorsque vous jouez une créature sans famille, elle devient d'une famille aléatoire.";
}
...
function Effect68() {
    ...
    this.desc = "Votre niveau de recrutement augmente un tour plus tôt, mais vous gagnez moins de pièces d'or.";
}
...
function Effect69() {
    ...
    this.desc = "Vos créatures peuvent être jouées comme des Sortilèges sur des créatures d'autres familles pour changer leur famille et leur conférer +1/+1.";
}

function Effect70() {
    this.run = async (sender, args, doAnimate) => {
        const cards = Array.from(document.getElementById("shop").children)
            .filter(c => c.card && c.classList.contains("frozen"))
            .map(c => c.card)
            .filter(card => card.species !== "Sortilège");
        if (cards.length) {
            if (doAnimate)
                await effectProcGlow(sender);
            cards.forEach(card => boostStats(card, shopTier, shopTier, true));
            await sleep(400);
        }
    };
    this.scaling = (c, t) => {
    ...
    this.desc = "A la fin de chaque tour, confère +X/+X aux créatures gelées, où X est votre niveau de recrutement.";
}
...
function Effect71() {
    this.run = async (sender, args, doAnimate) => {
        if (sender.hp <= 10) {
            if (doAnimate)
                await effectProcGlow(sender)
            troops[0].filter(c => c).forEach(c => boostStats(c, 2, 0, doAnimate));
            if (troops[0].some(c => c))
                await sleep(400);
            await enhanceShop(2, 0);
        }
    };
    this.scaling = (c, t) => {
        return [0, fluctuate(t.filter(e => e.shield).length * 2, 0, 1.5), 0, 0];
...
function Effect1701() {
    ...
    this.desc = "<em>Recrue :</em> Perdez 1 PV.";
}

function Effect1702() {
    this.run = async (sender, args, doAnimate) => {
        let name = players[0].src.substring(players[0].src.indexOf("/") + 1, players[0].src.indexOf("."));
        if (sender === args[0].card && players[0].hp < createCard(name).hp) {
            if (doAnimate)
                await effectProcGlow(sender);
            await boostStats(sender, 1, 1, doAnimate);
        }
    ...
    this.desc = "<em>Recrue :</em> S'il vous manque des PV, gagne +1/+1.";
}
...
function Effect1703() {
    ...
    this.desc = "Au début de chaque tour, vous perdez 2 PV et cette créature gagne +2/+2.";
}
...
function Effect1704() {
    this.run = async (sender, args, doAnimate) => {
        if (args[0] === sender && sender.hp <= 0) {
            if (doAnimate)
                await effectProcGlow(sender);
            let side = args[2][0].concat(args[2][1]).includes(sender);
            await boostStats(players[side ? args[4] : args[5]], 0, 1, doAnimate);
            drawPlayers();
        }
    };
    this.scaling = (c, t) => {
    ...
    this.desc = "<em>Dernière volonté :</em> Récupérez 1 PV.";
}
...
function Effect1705() {
    ...
    this.scaling = (c, t) => {
        return [3 * (t.filter(e => e && e.species === "Vampire").length > 1), 0, 0, 0];
    };
    this.battleValue = (c, t) => {
        return 0;
    };
    this.desc = "Lorsque vous perdez des PV, récupérez jusqu'à 1 PV ainsi perdu.";
}
...
function Effect1706() {
    this.run = async (sender, args, doAnimate) => {
        if (sender === args[0].card) {
            if (doAnimate)
                await effectProcGlow(sender);
            let n = Math.min(3, players[0].hp - 1);
            await boostStats(players[0], 0, -n, doAnimate);
            drawPlayers();
            const targets = troops[0].filter(c => c && c.species === "Vampire" && c !== sender);
            if (targets.length) {
                targets.forEach(c => boostStats(c, 2, 2, doAnimate));
                await sleep(400);
            }
        }
    };
    this.scaling = (c, t) => {
        return [0, 0, 0, 3 * t.filter(e => e && e.species === "Vampire").length];
    };
    this.battleValue = (c, t) => {
        return 0;
    };
    this.desc = "<em>Recrue :</em> Perdez 3 PV, puis confère +2/+2 aux autres Vampires alliés.";
}
...
function Effect1707() {
    ...
    this.scaling = (c, t) => {
        return [0, 8 * (t.filter(e => e && e.species === "Vampire").length > 4), 0, 0];
    };
    this.battleValue = (c, t) => {
        return 0;
    };
...
function Effect1708() {
    this.run = async (sender, args, doAnimate) => {
        if (sender.hp > 0) {
            const side = args[0][0].concat(args[0][1]).includes(sender);
            const player = side ? args[2] : args[3];
            const enemy = side ? args[3] : args[2];
            const diff = players[enemy].hp - players[player].hp;
            if (diff > 0) {
                if (doAnimate)
                    await effectProcGlow(sender);
                await boostStats(sender, diff, diff, doAnimate, true, true);
            }
        }
    };
    this.scaling = (c, t) => {
    ...
    this.desc = "<em>Frappe préventive :</em> Si vous avez moins de PV que votre ennemi, gagne définitivement +X/+X, où X est la différence.";
}

function Effect1709() {
    this.run = async (sender, args, doAnimate) => {
        if (args[0].card !== sender && args[0].card.species === "Vampire" && findDOMCard(sender).parentElement.parentElement.classList.contains("board") && players[0].effect1710) {
            const targets = shuffle(troops[0].filter(c => c && c.species === "Vampire"));
            if (targets.length) {
                if (doAnimate)
                    await effectProcGlow(sender);
                for (let i = 0; i < players[0].effect1710; i++)
                    await boostStats(choice(targets), 3, 3, doAnimate);
            }
        }
    };
    this.scaling = (c, t) => {
        return [0, 7 * t.filter(e => e && e.species === "Vampire").length, 0, 0];
    };
    this.battleValue = (c, t) => {
        return 0;
    };
    this.desc = "Lorsque vous revendez un Vampire, confère +3/+3 à un Vampire allié aléatoire pour chaque PV perdu ce tour-ci.";
    this.dynamicDesc = (c) => `<em>(Actuellement ${players?.[0]?.effect1710 ?? 0})</em>`;
}

function Effect1710() {
    this.run = async (sender, args, doAnimate) => {
        if (!sender.effect1710)
            sender.effect1710 = 0;
        sender.effect1710 += args[0];
    };
    this.scaling = (c, t) => {
        return [0, 0, 0, 0];
    };
...
function Effect1713() {
    this.run = async (sender, args, doAnimate) => {
        if (findDOMCard(sender).parentElement.parentElement.classList.contains("board") && players[0].effect1710 && players[0].effect1710 >= 3) {
            if (doAnimate)
                await effectProcGlow(sender);
            sender.deathtouch = true;
            await boostStats(sender, 0, 0, doAnimate);
    ...
    this.desc = "A la fin de chaque tour, si vous avez perdu au moins 3 PV ce tour-ci, acquiert le <em>Contact mortel</em> jusqu'au prochain tour.";
    this.dynamicDesc = (c) => `<em>(Actuellement ${players?.[0]?.effect1710 ?? 0})</em>`;
}

function Effect1714() {
    this.run = async (sender, args, doAnimate) => {
        if (sender.deathtouch) {
            delete sender.deathtouch;
            await boostStats(sender, 0, 0, doAnimate);
        }
    };
    this.scaling = (c, t) => {
...
function Effect1715() {
    this.run = async (sender, args, doAnimate) => {
        if (args[0].card === sender) {
            if (doAnimate)
                effectProcGlow(sender);
            await chooseTarget((target) => {
                const buff = players[0].effect1710 ?? 0;
                boostStats(target, buff, buff, doAnimate);
            }, {
                area: "board"
            }, sender);
        }
    ...
    this.scaling = (c, t) => {
        return [0, 0, 0, 10 * (t.filter(e => e && e.species === "Vampire") > 3)];
    };
    this.battleValue = (c, t) => {
        return 0;
    };
    this.desc = "<em>Recrue :</em> Confère +X/+X à une créature alliée ciblée, où X est le nombre de PV perdus ce tour-ci.";
    this.dynamicDesc = (c) => `<em>(Actuellement ${players?.[0]?.effect1710 ?? 0})</em>`;
}
...
function Effect1716() {
    this.run = async (sender, args, doAnimate) => {
        if (sender === args[0].card) {
            ...
            await boostStats(players[0], 0, -n, doAnimate);
            drawPlayers();
            await boostStats(players[0], 0, 1, doAnimate);
            drawPlayers();
        }
    };
    this.scaling = (c, t) => {
        return [0, 0, 0, 4 * (t.filter(e => e && e.species === "Vampire") > 2)];
    };
    this.battleValue = (c, t) => {
        return 0;
    };
    this.desc = "<em>Recrue :</em> Perdez 1 PV, puis récupérez 1 PV.";
}
...
function Effect1717() {
    ...
    this.toBack = true;
    this.desc = "Lorsqu'une créature ennemie meurt, gagne définitivement +1/+0.";
}
...
function Effect1718() {
    ...
    this.desc = "Au début de chaque tour, si vous avez gagné le dernier combat, perdez 1 PV. Sinon, récupérez 1 PV.";
}
...
function Effect1719() {
    this.run = async (sender, args, doAnimate) => {
        ...
        drawPlayers();
        const buff = players[0].effect1710 ?? 0;
        await boostStats(args[0].card, buff, buff, doAnimate);
    };
    this.scaling = (c, t) => {
        return [0, 0, 0, 0];
    };
    ...
    this.desc = "Perdez 2 PV. Confère +X/+X à la créature alliée ciblée, où X est le nombre de PV perdus ce tour-ci.";
    this.dynamicDesc = (c) => `<em>(Actuellement ${players?.[0]?.effect1710 ?? 0})</em>`;
}
...
function Effect1720() {
    ...
    this.desc = "Confère \"Lorsque vous perdez des PV, gagne +1/+0 ou +0/+1.\" à la créature alliée ciblée. Elle devient un Vampire. Perdez 2 PV.";
}
...
async function boostStats(card, atk, hp, doAnimate, preserveHP, permanent) {
    ...
    if (currentScene === "shop" && card === players[0] && hp < 0) {
        const nGained = troops[0]
            .filter(c => c)
            .map(c => c.effects)
            .flat()
            .filter(e => e.id === 1705)
            .length;
        if (nGained)
            await boostStats(players[0], 0, Math.min(nGained, -hp), doAnimate);
        selfHarm -= hp;
        if (selfHarm >= 50 && !JSON.parse(window.localStorage.getItem("Achievement__Masochisme")) && !achievementsWaiting.includes("Masochisme"))
            achievementsWaiting.push("Masochisme");
        await broadcastShopEvent("lifeloss", [-hp]);
    }
}
...
function getAchievementDesc(a) {
    switch (a) {
        ...
        case "Polymorphie":
            return "Posséder un Changeforme masqué de statistiques supérieures à 40/40.";
        case "Hyperspécialisation":
            return "Gagner une partie en n'achetant que des sortilèges et des créatures de la même famille."
        case "Honneur du guerrier":
            return "Gagner une partie sans jouer de sortilège."
        case "Paresseux":
